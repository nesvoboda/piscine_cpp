...
int main(void)
{
	...
	*/
	...
	out("Intern | Constructor");

	Intern i1 = Intern();

	out("Intern | Copy constructor");

	Intern i2 = Intern(i1);

	out("Intern | Assignment operator");

	i2 = i1;

	out("Intern | Destructor");
	
	{
		Intern i4 = Intern();
	}

	out("Intern | makeForm -> ShrubberyCreationForm");
	Form *a1 = i2.makeForm("ShrubberyCreationForm", "dummy");

	out("Intern | makeForm -> RobotomyRequestForm");
	Form *a2 = i2.makeForm("RobotomyRequestForm", "dummy");

	out("Intern | makeForm -> PresidentialPardonForm");
	Form *a3 = i2.makeForm("PresidentialPardonForm", "dummy");

	tom.signForm(*a3);

	eo("dummy has been pardoned by Zafod Beeblebrox\n\
Tom, bureaucrat grade 3 executes Presidential Pardon Form, form, grade to sign: 25, grade to execute: 5");
	tom.executeForm(*a3);

	check(i2.makeForm("Whatever", "dude") == nullptr);
